...
actor CodexLogUsageScanner {
    ...
    struct RawUsage: Sendable {
        ...
        init(json: [String: Any]) {
            func int(_ keys: String...) -> Int? {
                for key in keys {
                    if let value = json[key] { return UsageTokenCount.read(value, provider: "codex") }
                }
                return nil
            }
    ...
    static func aggregate(
        ...
        for event in events where event.timestamp >= since {
            ...
            let resolvedRates = resolution.rates
            ...
            let appliesCodexFastTier = resolution.isFastAlias ? resolution.hasBaseRates : event.isFast
            let eventCost = cost(
                rates: rates,
                event: event,
                model: resolution.rateModel,
