...
actor ClaudeLogUsageScanner {
    ...
    /// Internal rather than private so the personal-fork account tests can assert that the two pinned
    /// Claude tiles never share a parse-cache namespace.
    func parseCacheIdentity() -> String {
        ...
        let roots = Set(configuredRoots.map { $0.resolvingSymlinksInPath().standardizedFileURL.path })
            .sorted()
            .joined(separator: "\n")
        // The Cowork flag changes which files are scanned without changing any root above, and the
        // scanner replaces a whole identity's cache with the file set it just saw. Two scanners
        // sharing an identity but disagreeing on the flag would evict each other's records every
        // refresh, so it has to be part of the key. Only appended when off, keeping the default
        // identity byte-identical to upstream's.
        let cowork = includeCoworkSandboxes ? "" : "\ncowork=off"
        return "home=\(home)\nroots=\(roots)\(cowork)"
    }
