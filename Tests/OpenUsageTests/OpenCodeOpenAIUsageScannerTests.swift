...
final class OpenCodeOpenAIUsageScannerTests: XCTestCase {
    ...
    private func scanner(auth: String, rows: String, sqlite: OpenCodeFakeSQLite? = nil) -> OpenCodeOpenAIUsageScanner {
        let database = sqlite ?? OpenCodeFakeSQLite(data: ["/oc/opencode.db": rows])
        return OpenCodeOpenAIUsageScanner(
    ...
    private func channelScanner(
        ...
        rows: [String: String]
    ) -> (OpenCodeOpenAIUsageScanner, OpenCodeFakeSQLite) {
    ...
    func testOAuthCreationTimeBoundsOnlyTheV2Branch() {
        ...
        let sql = OpenCodeOpenAIUsageScanner.dataSQL(cutoffMs: 123, oauthSinceMs: 456)
        ...
        XCTAssertFalse(OpenCodeOpenAIUsageScanner.dataSQL(cutoffMs: 123).contains(">= 456"))
    }
    ...
    func testScanPassesTheCredentialCreationTimeIntoTheQuery() async {
        ...
        let databaseBacked = OpenCodeOpenAIUsageScanner(
            authStore: openCodeAuthStore(sqlite: sqlite),
            sqlite: sqlite,
            databasePaths: { ["/oc/opencode.db"] }
        )
    ...
    func testVariantSQLSelectsOnlyTheTablesItWasGiven() {
        let v1 = OpenCodeOpenAIUsageScanner.dataSQL(cutoffMs: 123, tables: .v1)
        XCTAssertTrue(v1.contains("FROM message"), v1)
        XCTAssertFalse(v1.contains("session_message"), v1)

        let v2 = OpenCodeOpenAIUsageScanner.dataSQL(cutoffMs: 123, tables: .v2)
    ...
    func testMessageTablesProbeParsing() throws {
        func probe(_ output: String) throws -> OpenCodeOpenAIUsageScanner.MessageTables? {
            try OpenCodeOpenAIUsageScanner.messageTables(
