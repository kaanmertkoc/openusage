// ============================================================================
// claude-work: a copy of plugins/claude/plugin.js pinned to a SECOND Claude
// Code account (CLAUDE_CONFIG_DIR=/Users/kaankoc/.claude-work).
//
// The OpenUsage process env is shared by every plugin and does NOT inherit the
// shell alias's CLAUDE_CONFIG_DIR, so this account cannot be selected via env.
// It is hardcoded below in ACCOUNT_CONFIG_DIR.
//
// To re-sync after upstream changes to plugins/claude/plugin.js:
//   1. cp plugins/claude/plugin.js plugins/claude-work/plugin.js
//   2. re-apply the 6 edits tagged "[claude-work]" (search the file for the tag)
// ============================================================================
(function () {
  const DEFAULT_CLAUDE_HOME = "~/.claude"
  // [claude-work] Absolute path (no "~") so the keychain hash matches what
  // Claude Code stores. The shell alias MUST set the same absolute value:
  //   CLAUDE_CONFIG_DIR=/Users/kaankoc/.claude-work
  ...
  function getClaudeHomePath(ctx) {
    // [claude-work] Pinned to this account's dir; ignore the shared process env.
    return ACCOUNT_CONFIG_DIR
  }

  function getClaudeHomeOverride(ctx) {
    // [claude-work] Always return the pinned dir so ccusage reads this account's
    // logs (passed through as homePath -> CLAUDE_CONFIG_DIR for the subprocess).
    return ACCOUNT_CONFIG_DIR
  }
  ...
  function computeKeychainHashSuffix(ctx) {
    // [claude-work] Mirrors upstream Claude Code (decompiled from the binary):
    //   "-" + sha256(CLAUDE_CONFIG_DIR.normalize("NFC")).slice(0, 8)
    // Hashed unconditionally from the PINNED dir (never the shared env), so this
    // tile always targets the work account's keychain entry.
    ...
    const normalized =
      typeof ACCOUNT_CONFIG_DIR.normalize === "function"
        ? ACCOUNT_CONFIG_DIR.normalize("NFC")
        : ACCOUNT_CONFIG_DIR
  ...
  function getClaudeKeychainServiceCandidates(ctx) {
    // [claude-work] Hashed entry ONLY. The legacy unhashed "Claude Code-credentials"
    // belongs to the DEFAULT account, so we must never fall back to it here — doing
    // so would make this tile silently mirror the personal account's usage.
  ...
  function queryTokenUsage(ctx, homePath) {
    ...
    // [claude-work] The host infers the ccusage provider from the plugin id, but
    // "claude-work" is not a known id, so pass it explicitly to use Claude's runner.
    const queryOpts = { provider: "claude", since: sinceStr }
    if (homePath) {
  ...
  // [claude-work] id matches plugins/claude-work/plugin.json.
  globalThis.__openusage_plugin = { id: "claude-work", probe, _resetState }
})()
